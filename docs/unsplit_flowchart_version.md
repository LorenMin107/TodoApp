...
sequenceDiagram
    ...
    App->>App: Generates verification token
    App->>DB: Stores user with verification token
    App->>User: Sends verification email
    User->>Browser: Clicks verification link in email
    Browser->>App: GET /auth/verify-email?token=xyz
    App->>DB: Verifies token and marks email as verified
